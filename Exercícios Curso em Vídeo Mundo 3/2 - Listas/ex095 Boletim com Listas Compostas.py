...
for ind_aluno, aluno in enumerate(ficha):
    # Mostre o índice do aluno + 1 (enumerate trabalha começando com 0, para deixar mais fácil para os usuários, adicionamos +1 para que o primeiro aluno seja 1, não 0), o elemento 0 (que representa o nome dele)
    # (O aluno que ele deve mostrar é ordenado pela variável aluno), e por fim o elemento 2 (a média dele):
...
while True:
    ...
    if 0 < opc <= len(ficha):
        # Como as listas começam no 0, colocamos - 1 para que o índice esteja correto novamente, pois na verdade o usuário está digitando o valor "errado" do aluno que ele quer ver, vamos supor que ele quer 
        # ver o primeiro aluno (que é 0, porém mostramos 1), quando ele digita 1, os dados do aluno que o software passaria seriam do aluno errado. Então corrigimos isso com o [opc - 1]:
        print(f'As notas de {ficha[opc - 1][0]} são ', end='')
        # Separe cada elemento da lista por vírgulas (transforme todos os elementos em strings para que isso funcione), mostre a média do aluno (Mesma técnica do - 1 já explicada anteriormente.):
